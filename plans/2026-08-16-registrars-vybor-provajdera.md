...
- [x] **Шаг 1: Падающий тест на правку ручного провайдера**
...
- [x] **Шаг 2: Прогнать — падает (сейчас поле ключа есть у всех)**
...
- [x] **Шаг 3: Правки `EditRegistrarModal`**
...
- Реализован целиком: **нет** — фазы 1–6 исполнены, функционально план закрыт (все
  acceptance criteria, кроме последнего — «зелёная сюита», — отмечены).
- Что осталось: фаза 7 — полный прогон и чистка. Открытый долг перечислен в самой фазе:
  визуальная приёмка (два зелёных чипа подряд, кегль буквы в аватаре), возврат фокуса на
  кнопку комбобокса, терпимость `providerMeta` к `null` (после Фазы 6 страховок
  `String(provider || "")` на местах вызова стало три: карточка и две в
  `EditRegistrarModal`).
