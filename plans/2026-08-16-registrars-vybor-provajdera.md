...
- [x] `npm test` зелёный (987 тестов в 94 файлах, `tsc --noEmit` чист); существующие тесты
...
- [x] **Шаг 1: Полный фронтовый набор** — `cd frontend && npx vitest run`: **987 зелёных в
      94 файлах** (было 979 — восемь новых тестов фазы, по два на каждое решение). Ни один
      соседний тест адаптации не потребовал.
...
- [x] **Шаг 3: Коммиты** — `86b1de1`, `4ecc102`, `eb4227d`, `8590a50` (по одному на решение)
      плюс `2a814d8` по само-ревью: `needsClientIp` и `apiUserField` повторяли связку «гейт
      `hasApi` + запись каталога» — сведена в приватный `apiEntry`, потому что причина гейта
      у обоих одна (у непризнанного провайдера не бывает НИ ОДНОГО свойства API).
...
- [x] **`providerMeta` терпит `null`/`undefined` сама.** Страховок `String(provider || "")`
  ...
  error boundary: одна порченая строка уносила бы всю вкладку Registrars, а не одну
  ...
  **Оговорка, чтобы её не цитировали неверно:** колонка `provider` **НЕ** nullable
  (`registrar_account.py` — `nullable=False`, схема ответа — `provider: str`), так что
  штатный `null` от сервера прийти не может. Терпимость здесь — про битый или частичный
  ответ и про `undefined` у недособранного объекта, плюс снятие с вызывающих обязанности
  писать свой `String(... || "")`. Первая редакция этого абзаца и JSDoc ссылалась на
  «nullable-колонку» — финальное ревью поймало, что это ложный факт о схеме; в кодовой
  базе, где на комментарии-«почему» потом опираются, такой факт живёт дольше и дороже,
  чем недостающий `?? ""` (в отличие от `api_user`, который nullable по-настоящему).
...
### Правки по финальному ревью всей ветки  `[x]`

Ревью одобрило ветку к слиянию. Один блокер и три правки:

- [x] **БЛОКЕР: симлинк `frontend/node_modules` был закоммичен в ветку.** Уехал в индекс
  через `git add -A` контроллера ещё в `b33793f` и прожил 28 коммитов. Это симлинк на
  абсолютный путь конкретной машины: после слияния у каждого, кто сделает checkout, на
  его месте оказался бы битый симлинк, а `npm ci` упал бы на `ENOTDIR` либо записал
  пакеты сквозь него в чужой каталог. Проскочило потому, что правило `.gitignore` было
  `node_modules/` **со слэшем** — это шаблон каталога, а симлинк каталогом не является.
  Снят из индекса, правило переписано без слэша (ловит оба случая, следующий worktree не
  повторит). Коммит `f5beabd`.
- [x] **Ложный факт в обосновании терпимости к `null`.** JSDoc и план утверждали
  «колонка `provider` nullable». Она `nullable=False`. Исправлено везде — см. оговорку в
  «Накопленном долге» Фазы 7.
- [x] **`buildProviderList` был единственным местом модуля без терпимости** — звал строгий
  `normalizeProvider` напрямую. Запись с `provider: null` рисовала карточку нормально, но
  **не давала открыть форму добавления** (`Cannot read properties of null`). Новый путь
  отказа, которого до ветки не было: раньше форма списка аккаунтов вообще не получала.
  Ослаблена сигнатура, а не только выражение: тип описывает чужие данные.
- [x] **`providerMeta` переведена на собственный `apiEntry`** — расписанная руками связка
  «гейт + запись каталога» была третьей репликой паттерна, ради устранения которого
  хелпер и заводился. Плюс тултип `ProviderLabel` переведён на русский: это единственное
  место фичи, которое даёт **инструкцию**, а не подпись, и адресат у неё ровно один — тот,
  кто пришёл выяснять, почему его Hostiq считается ручным.

Коммит правок: `6bce23b`. Сюита после них — **988** тестов в 94 файлах, `tsc` чист.
...
- Реализован целиком: **да** — все семь фаз исполнены, все acceptance criteria закрыты,
  сюита зелёная (988 тестов в 94 файлах, `tsc --noEmit` чист). Из кода не осталось ни одного
