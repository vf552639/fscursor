...
- [x] `docs/FASTPANEL_CLI.md` описывает каждую команду с пометкой «сверено вживую» / «не сверено»
      (по решению — обезличенные фрагменты реальной формы, а не сырой вывод целиком).
- [x] Кнопка «Проверить на сервере» в `DomainDetailModal` за одну SSH-сессию снимает
      SSL / FTP / PHP / сайт / БД / логи и пишет результат в БД.
- [x] Провалившаяся проверка не стирает и не молодит последний хороший снимок.
- [x] Ни одно поле overview не выглядит здоровым, если проверки не было или она протухла.
- [x] FTP-доступ в модалке пригоден к использованию: хост + порт + логин + пароль.
- [x] Provision больше не теряет пароли FTP и БД — они уходят в блобы сразу в `onResult`.
- [x] «Sync Domains» не возвращает 404, а показывает сравнение сервер ↔ SDMP.
- [x] Плейнтекст-секретов нет ни в `DomainFacts`, ни в argv команд чтения, ни в audit.
...
### Фаза 0 — Discovery на живом сервере → документация CLI  `[x]`

**Выполнено** (2026-08-16): discovery-скрипт написан и прогнан на живом FastPanel 2 под
root (exit 0); `docs/FASTPANEL_CLI.md` написан по реальному выводу — по каждой команде argv,
форма json+текст, коды возврата, поведение «уже существует», пометка «сверено вживую
(2026-08-16)» / «не сверено». Разделы «Что мы отправляем сегодня» (инвентарь всех команд из
`fastpanel.rs` + `fastpanel_install.rs`) и «Правила» (`pty:false`, `q()`, `opaque_exit`,
каскад json→текст→«не знаем») на месте. Ссылки прописаны в `CLAUDE.md` (таблица «где что
искать») и `docs/TECH_STACK.md`. Ключевые находки discovery отражены: команды только под
root; `--json` после подкоманды; `sites list --json` как богатый источник (php из
`main_backend`); FTP-список глобальный и без пароля; **баг `database` (ед.ч.) vs
`databases` (мн.ч.)** — заведён follow-up `plans/2026-08-16-fastpanel-database-vs-databases.md`;
`php`-команды нет; раскладка логов/конфигов; подкоманды `certificates`.
**Отклонение от исходного критерия:** сырой вывод discovery **не приложен файлом** — в нём
боевые домены/логины/пути; вместо этого в доку вставлены обезличенные фрагменты реальной
формы (плейсхолдеры `example.com`/`example_usr`/`example_ftp`/`exampledb`/`203.0.113.10`),
структура/поля/коды сохранены точь-в-точь. Обезличивание перепроверено grep'ом.
...
- Реализован целиком: **да** (2026-08-16). Все фазы 0–5 закрыты, каждая прошла двухстадийное
  ревью (соответствие спеке + качество кода); финальное интеграционное ревью ветки — без
  Critical/Important, контракт `DomainFacts` совпадает через все слои (Rust → HTTP → БД → TS).
- Проверки: backend `pytest` (16 новых + регрессия), frontend `npm test` (933), desktop
  `cargo test` (256), `tsc`/`cargo clippy` чисто. Discovery прогнан на живом сервере под root;
  парсеры сверены с реальным выводом; на командном уровне подтверждено: срок SSL из
  `certificate{}` совпадает с `openssl` байт-в-байт, пути логов существуют, `databases`/FTP/sites
  JSON парсятся верно.
- Остаётся долг (вне контракта, к ручной приёмке пользователем): **полный GUI-прогон** —
  собрать десктоп, нажать «Проверить на сервере», убедиться в записи `fp_checked_at` и входе
  по FTP заданными кредами; проверить провал (погасить сервер) → `fp_check_error` + протухшая
  свежесть.
- Найденные в discovery расхождения версии FastPanel вынесены отдельным планом
  `plans/2026-08-16-fastpanel-database-vs-databases.md` (баг `database`→`databases` в provision;
  туда же — путь серта `read_ssl_info` смотрит `/etc/letsencrypt/live`, а FastPanel держит серты
  в `/var/www/httpd-cert/` — SSL функционально спасён обогащением из `certificate{}`, но точный
  issuer через openssl не читается). Мутации из плана не трогались — здесь только чтение.
