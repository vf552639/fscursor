...
> ⚠️ **Пути ниже устарели: после плана №4 страница разобрана**
> (`2026-08-11-domains-razbivka-god-komponenta.md`). Ссылки вида «в
> `pages/Domains.tsx`» и номера строк в ней читай как запись о том, где это было
> на момент работы, а не как указатель. Сегодня: `pages/Domains.tsx` — 399-строчный
> оркестратор; разметка — `components/domains/` (`DomainTable`, `DomainRow`,
> `DomainStatusBadge`, `DomainFilters`, `DomainStats`, диалоги `AddDomainModal`,
> `BulkAddDialog`, `ProvisionDialog`, `AssignServerDialog`,
> `AssignCloudflareDialog`, баннеры); поведение — `hooks/` (`useDomainFilters`,
> `useDomainSort`, `useBulkProvision`, `useCloudflareBind`, `useLiveOrAway`,
> `useNow`); чистые правила — `lib/` (`domainStatus`, `domainExpiry`,
> `cfZoneMatch`). Переименования: `components/StatusBadge.tsx` →
> `components/domains/DomainStatusBadge.tsx`; `Th` → `PlainTh`/`SortableTh` внутри
> `DomainTable.tsx`.
...
- Результат (`BulkProvisionOutcome`) отдать наверх новым обязательным пропом
  ...
  пользователя. `DesktopWorkspace` уже держит `bulkReportQueue` и
  `BulkProvisionReportModal` — подключаемся к ним, второй очереди не заводим.
  (По ревью очередь сужена до `useShowOnceQueue<BulkProvisionReport>`: полный
  отчёт в неё кладётся структурно, а достать оттуда пароль уже не даёт
  компилятор.)
...
- Реализован целиком: да (фазы 1–5, все acceptance criteria закрыты).
  ...
  - Фаза 5: `npx tsc --noEmit` чист, `npm test` — 65 файлов / 630 тестов
    зелёные (удалённое покрыто не было; 635 на конце ветки — это те же тесты
    плюс пять, добавленных двумя раундами ревью после фазы 5). Grep по
  ...
  - Последним раундом тост получил третий тон — `warn`. Полууспех («Zone
    created, but the result could not be saved on the server») врал в обе
    стороны: под зелёной галочкой всё в порядке, хотя сервер о зоне не знает и
    следующая проверка идемпотентности решит неверно, а под красным крестом
    пользователь идёт создавать зону второй раз. Тем же тоном теперь говорят
    шаги прогресса из `WARNING_STEPS` (`*_failed`, `creds_unparsed`,
    `ssl_skipped_*`, `firewall_warning`, `bulk_failed`) и невыполненная
    `sdmp://`-ссылка («ignored — nothing was run» вместо «✓ Deep link: …»).
    Забытую строчку в `WARNING_STEPS` сторожит тест: каждая метка шага со
    словами «, but » обязана быть предупреждением.
...
- Долг НЕ этой чистки, но найденный при её ревью, — гонка «переназначение
  сервера во время массового прогона». Записан, чтобы не искать заново; кода по
  нему не трогали.
  - Механика: `run_bulk_provision` решает, на какой сервер идти, ЛЕНИВО, на
    каждой итерации — `provision_domain_inner` открывает SQLCipher-кэш заново и
    читает `domains.server_id` → `servers` в тот момент, когда цикл дошёл до
    домена (`desktop/src-tauri/src/commands/provision.rs:1061-1077`, цикл
    `:1373`).
  - Полная цепочка условий: идёт прогон → пользователь переназначает сервер
    (это чистый HTTP, кэша он не касается) → пользователь вдобавок дёргает
    любую другую Tauri-команду (⚙ на другом домене, Set NS, действие
    Cloudflare), чей `invokeSynced` → `syncLocalCache` перепишет строки кэша.
    Фонового таймера синхронизации нет, так что без третьего шага гонка не
    выстреливает. После него домены, до которых прогон ещё не дошёл, поедут на
    НОВЫЙ сервер, пока пользователь считает прогон привязанным к старому.
  - Ничего не рвётся: write-back пишет тот сервер, на котором реально
    отработали, — это сюрприз, а не потеря, и он был возможен и до этой ветки.
  - Два кандидата на исправление, если сюрприз решат закрыть: гасить assign
    только для ПЕРЕСЕЧЕНИЯ выделения с идущим набором (предикат уже есть —
    `alreadyProvisioning`), либо снимать снимок `server_id` каждого домена в
    Rust до цикла. Первый чинит UI, второй — саму лень чтения.
- Долг, заведённый этой чисткой: `useZoneDetails` (`api/cloudflare.ts`) остался
  БЕЗ производственных вызывающих — его единственным был удалённый
  ...
  - ~~Его естественный потребитель — план №2 (обогащение read-only-обзора
    полями зоны). Если план №2 им не воспользуется, хук надо будет удалить там
    же.~~ **Указание отозвано 2026-08-11**, а не оставлено висеть: планы №2 и
    №3 прошли, ни один хук не взял и не удалил, и удалять его теперь незачем —
    ниже два конкретных факта, которых на момент записи не было.
  - **План №2 отказался осознанно** (его фаза 5): дом хуку был бы на
    read-only-обзоре карточки домена, а `overview` — вкладка, открытая ВСЕГДА.
    Значит цена — пагинированный вызов `cf_list_zones`, только в десктопе, на
    каждое открытие карточки, ради нескольких справочных полей. Это не «забыли»,
    это «дорого».
  - **План №3 им стать не мог** — по устройству, а не по решению: привязке нужен
    ВЕСЬ список зон, чтобы искать по имени, а `useZoneDetails` выбирает ОДНУ
    зону по уже известному `zone_id`. Матчинг вдобавок идёт вне React
    (`api/cfAutoBind.ts`), а это хук.
  - Итого: хук рабочий, но без UI. Это живой долг «мёртвый экспорт», а не
    невыполненное поручение; сносить его в рамках уборки документации было бы
    удалением работающего кода не на своём месте. Решать судьбу — вместе с
    задачей, которой понадобятся поля зоны (или явным решением их не показывать).
