...
- [x] **2. Громкий провал** — `extra="forbid"` на `ServerCreate/Update`, `CloudflareAccount*`,
  ...
  - [x] Побочная находка, закрытая здесь же: дефолтный 422 FastAPI возвращает `input` —
        ...
        (`api/client.ts`), в тост и в кэш мутаций, десктоп — в `ApiError::Http`, а прокси
        — в лог. `validation_error_without_extra_input` в `app/main.py` снимает `input`
        у ошибок `extra_forbidden` **и у любых ошибок, где `input` — контейнер**: у
        `missing` и `model_attributes_type` pydantic кладёт туда всё тело запроса
        целиком, так что одного `extra_forbidden` не хватало (находка spec-ревью).
        Скалярный `input` объявленных полей сохранён — на это есть свой тест.
        Мутационная проверка: старый узкий фильтр роняет 3 случая, отсутствие фильтра —
        9, снятие `input` у всех — тест на диагностику.
