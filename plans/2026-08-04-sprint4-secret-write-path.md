...
- [x] **2. Громкий провал** — `extra="forbid"` на `ServerCreate/Update`, `CloudflareAccount*`,
  ...
  - [x] Побочная находка, закрытая здесь же: дефолтный 422 FastAPI возвращает `input` —
        ...
        конвенция, что у десктопного `audit_redact.rs`, плюс `_b64` и `_blob_id`:
        `auth_key_b64` короче минимума отдаёт `string_too_short` с ключом внутри, а
        `ssh_password_blob_id` с плейнтекстом — `uuid_parsing` с паролем).
