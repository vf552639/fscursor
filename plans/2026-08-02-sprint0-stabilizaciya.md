...
## Задача 10 (сверх плана): починить чтение `/tasks` и `/domains` во фронте

Возникла из разбора долга `tsc`: три ошибки TS2339 оказались не шумом типизации, а **живым
багом**. `GET /servers` отдаёт пагинированный `ServerListResponse` (`{items: [...]}`), а
`GET /tasks` (`List[TaskLogResponse]`) и `GET /domains` (`list[DomainResponse]`) — голый массив.
Паттерн `.items` был скопирован с серверов на задачи и домены; на массиве `.items` даёт
`undefined`, срабатывает `|| []`, и данные молча терялись — без ошибок в консоли.

**Что было сломано:**

| Место | Симптом |
|---|---|
| `Activity.tsx:13` | таблица задач всегда пуста, все 4 счётчика показывали 0 |
| `Dashboard.tsx:31` | счётчик доменов всегда 0 |
| `Dashboard.tsx:34` | список последних задач пуст |

**Files:**
- Modify: `frontend/src/pages/Activity.tsx:13`, `frontend/src/pages/Dashboard.tsx:31,34`
- Modify: `frontend/src/pages/Activity.test.tsx` (мок кодировал тот же неверный контракт)

- [x] **Шаг 1: Подтвердить контракт по бэкенду** — `backend/app/api/routes/tasks.py:19`
  (`response_model=List[TaskLogResponse]`), `domains.py:35` (`list[DomainResponse]`),
  `servers.py:22` (`ServerListResponse`).
- [x] **Шаг 2: Прочесать фронт на тот же паттерн** — `grep -rn "?\.items" frontend/src`.
  Из 7 совпадений 4 корректны (все — серверы), 3 сломаны.
- [x] **Шаг 3: Заменить `q?.items || []` на `q ?? []`** в трёх местах.
- [x] **Шаг 4: Прогнать `tsc`** — TS2339 больше нет (0), общее число ошибок 61 → 53.
- [x] **Шаг 5: Прогнать `npm test`** — упал `Activity.test.tsx`: мок `useTaskLogs` возвращал
  `{items: []}`, то есть тест фиксировал баг как норму. Мок приведён к реальному контракту
  (`data: []`); мок `useServers` оставлен как `{items: []}` — он верен.
- [x] **Шаг 6: Повторный прогон** — 6 файлов / 16 тестов passed. Коммит `c89c927`.

**Остаточный риск:** регресс-теста на сам баг нет. Текущий мок отдаёт пустой массив, поэтому
возврат к `?.items || []` тесты не поймают. Полноценный guard (мок с одной задачей + проверка,
что строка отрендерилась) вынесен вместе с типизационным долгом.
...
10. **Сверх плана:** починено чтение `/tasks` и `/domains` во фронте — три ошибки TS2339
    оказались живым багом (пустые таблица задач и счётчики дашборда). Коммит `c89c927`.
...
- **`npx tsc --noEmit` падает: 61 ошибка.** Это pre-existing долг, не связанный со спринтом —
  ...
  | ~~TS2339~~ | ~~3~~ | ~~`Activity.tsx:13`, `Dashboard.tsx:31,34`~~ — **оказались живым багом, починены (см. Задачу 10)** | — |

  После починки TS2339 осталось **53 ошибки** (ушли и несколько TS7006, ставших выводимыми).
  **Кандидат в отдельный спринт: типизационная гигиена frontend** — `@types/node` +
  `declare module 'argon2-browser'` для `crypto.ts`, затем массовая аннотация обработчиков,
  и обязательно **`tsc --noEmit` в CI**: без этого шага баг из Задачи 10 (который компилятор
  честно показывал) снова проскочит незамеченным.
