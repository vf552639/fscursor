...
/// среза userinfo и применяется к `host_port` и `tail` по отдельности. Обрезка
...
/// Со срезом userinfo первым от того же входа остаётся `https://1.2.3.4:8888`.
///
...
/// осознанно. Во-первых, кредов в нём уже нет (userinfo срезана раньше).
...
fn sanitize_panel_url(raw: &str) -> Option<String> {
    ...
    // обрезка до среза userinfo превращает пароль в «порт».
    let host_port = cut_at_control(host_port);
