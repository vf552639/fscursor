...
///
/// Управляющие символы (`\S+` их пропускает) обрезают токен, а не отвергают
...
/// на каждой цветной установке молча, а обрезка по первому управляющему
/// символу оставляет ровно то, что инсталлятор напечатал. Недоверенным
/// значение от этого быть не перестаёт: управляющий символ ВНУТРИ адреса рубит
/// authority (из `https://1.2.3.4:\x078888` остаётся `https://1.2.3.4:`), и
/// проверка «хост:порт» ниже отвергает такое значение целиком.
///
/// Отвергается (`None`) то, что после обрезки и среза userinfo не похоже на
...
fn sanitize_panel_url(raw: &str) -> Option<String> {
    // `split(..).next()` всегда даёт хотя бы один элемент; `unwrap_or_default`
    // тут только чтобы не тащить `unwrap` в код.
    let raw = raw
