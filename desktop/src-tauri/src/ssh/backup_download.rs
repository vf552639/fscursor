...
/// Суффикс недоделанного файла.
///
/// Публичный ради тестов (они живут отдельным крейтом и проверяют, что огрызок
/// не пережил отказа), а НЕ ради уборки: сканера осиротевших `.part` в продукте
/// сегодня нет вовсе, и обещать его комментарием нельзя.
///
/// Дыра, которую это оставляет, ровно одна и названа честно: `PartGuard` снимает
/// огрызок в `Drop`, то есть переживает и ранний `?`, и панику, и брошенный
/// фьючер, — но `SIGKILL` приложения `Drop` не выполняет, и гигабайтный
/// `<dest>.part` остаётся на диске навсегда. Уборка таких сирот записана в долг
/// плана (`plans/2026-08-19-bekapy-domena.md`, долг фазы 4): она требует места,
/// где вести список идущих выгрузок между запусками, а его сегодня нет.
...
pub enum DownloadError {
    ...
    ///
    /// **Здесь сырой stderr сервера — можно, и это не расхождение с решением
    /// фазы 2, а другая сторона одного правила.** В `warnings` его быть не
    /// может, потому что `warnings` — поле `Serialize`-структуры, которая
    /// уезжает наружу ВСЕГДА, в том числе на успешном прогоне, где читать в ней
    /// нечего: туда сырой текст попадал бы по умолчанию, россыпью, и однажды
    /// уехал бы вместе с результатом куда-нибудь ещё (в аудит, в лог, в
    /// поддержку). Текст отказа — противоположный случай: он существует ТОЛЬКО
    /// когда что-то сломалось, живёт ровно до показа человеку на его же машине
    /// и ни в какую структуру не складывается. А без него чинить нечего:
    /// «reading the archive failed (exit 1)» не отличает «нет места» от «файл
    /// исчез», и следующим шагом человек полезет на сервер руками.
    ///
    /// Границы у этого разрешения три, и все три соблюдены: текст обрезан до
    /// 300 символов (`trim_stderr`), пароля в нём быть не может по построению
    /// (в argv `cat <путь>` его нет вовсе — в отличие от `create`-команд
    /// FastPanel, ради которых написан `opaque_exit`), и на сервер он не
    /// уезжает ничем: в аудит идут только числа.
