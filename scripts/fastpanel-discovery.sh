...
#   каталог есть и пуст   в `-dir` нет записей, кроме `.` и `..`. Пустого вывода тут
#                         не бывает: `ls -la` всегда печатает и сами каталоги, и
#                         строку `total` — не ищи признак, которого нет. Число после
#                         `total` — сумма блоков перечисленного, оно зависит от ФС
#                         (замерено: APFS 0, ext4 2, tmpfs 4, overlayfs 8) и о
#                         пустоте не говорит ничего. `-files` пуст
...
if [ -n "$FP_DB" ]; then
  ...
  # гадать. С нейтрализованным `grep` код секции значит одно: сломался ли кто-то,
  # КРОМЕ `grep` — его собственный отказ (нет бинаря, битый шаблон) тоже съедается,
  # и это цена, которую мы платим сознательно: `grep` есть везде, включая busybox,
  # а отказ mysql — сценарий, ради которого секция и существует.
...
# Обрыв по `head` ловим двумя признаками, и второй — не перестраховка. Код 141
# приезжает, только если по SIGPIPE умер тот, чей код доживёт до `pipefail`, а
# `grep` мы выше обезвредили — и умереть может уже он. Замерено на этом же
# конвейере: 100 000 имён дают 141 (не успевает умереть слева никто другой), а
# 300 имён — код 0 при ровно 200 строках, потому что производитель успел дописать
# в буфер трубы и вышел нормально. Ровно этот умеренный перебор второй признак и
# ловит.
#
# Считаем РАЗОБРАННЫЕ имена, а не строки буфера: в буфер `run_sh` слил ещё и
# stderr, который через `head` не проходил, и 199 таблиц плюс одна строка ворчания
# дали бы ложное «список обрезан».
if [ "$BACKUP_TABLES_CODE" -eq 141 ] || [ "$backup_table_n" -ge "$MAX_LIST_LINES" ]; then
  note "имён разобрано $backup_table_n при потолке списка $MAX_LIST_LINES (код секции $BACKUP_TABLES_CODE) — список мог быть обрезан, таблиц могло быть больше"
  printf '\n'
fi
