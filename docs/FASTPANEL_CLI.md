...
1. **Полный набор команд доступен только под `root`.** Бинарь —
   ...
   *(Долг провенанса: верхнего списка `fastpanel --help` под **root** у нас в сыром выводе
   так и нет — снять при следующем discovery на живом сервере и вставить сюда. Сноска, а не
   блокер: вопрос «есть ли у CLI команда для логов» закрыт документацией вендора — нет,
   см. §6. Сторонний блог с листингом `mogwai --help` в разведке 2026-08-19 попался, но
   страницу открыть не удалось — на нём ничего не построено.)*
...
**Команды для чтения логов у CLI нет — и это проверено, а не предположено** (разведка
2026-08-19, `plans/2026-08-19-vkladka-logs-chtenie-hvosta.md`). База знаний вендора
(`kb.fastpanel.direct`): раздел CLI состоит из шести страниц (Introduction, Websites,
Databases, Users, Email, User transfer) — страницы про логи среди них нет; у `sites`
документированы ровно пять подкоманд (`list`, `create`, `delete`, `update`,
`batch-update`) — ни подкоманды, ни флага про логи; вендорская страница «логи сайта»
описывает модуль веб-панели и анализатор AWStats, путей к файлам не называет. Поэтому
**содержимое лога читается файлом напрямую** (`tail`), как SSL читается `openssl`, а
раскладка — `test -f`/`stat`. Побочно та же разведка **подтвердила первоисточником набор
из четырёх файлов** таблицы выше: frontend access/error (Nginx) и backend access/error
(Apache, PHP-FPM).

Ротация (`.1`, `.N.gz`) в снимок и в чтение хвоста не входит: `read_log_paths` строит
четыре кандидата по шаблону, а ротацию видно только `ls` — это правка контракта
`fp_facts`, отложенная отдельным планом.
...
| `tail -c 262144 <log> \| tail -n 200` (в обёртке с маркерами `#sdmp:`) | `read_log_tail` (`ssh/fastpanel_logs.rs`) | ⚠️ форма; живьём не прогонялась |

**Про чтение хвоста лога.** Это единственная команда чтения, которая идёт **мимо
FastPanel CLI по необходимости, а не по удобству**: команды для логов у CLI нет вовсе
(§6). Байтовый кап стоит в конвейере ПЕРВЫМ — `tail -n` по логу в гигабайты прочитал бы
файл целиком. Путь не берётся из запроса на веру: команда пускает только те пути, что
стоят в `fp_facts.logs[].path` этого домена, и отвергает остальные **до** коннекта
(`path_is_known` в `commands/domain_logs.rs`). Прочитанные строки **никуда не уезжают** —
ни в `fp_facts`, ни в SQLCipher-кэш, ни в аудит: в access-логе лежат чужие IP, URL и
user-agent'ы, а `fp_facts` — открытая JSON-колонка в Postgres.
