...
- **Server signals — two of them, independent:**
  ...
  |---|---|---|---|---|
  | alive / down | Celery Beat on the backend, TCP connect to `ip_address:ssh_port` | every 6 h, always | `last_check_at`, `last_check_ok`, `last_check_error`, `consecutive_failures` | 18 h (three missed runs) |
...
- **Server reachability monitoring flow** (`app/services/server_monitor.py` + `app/tasks/server_monitor_task.py`):
  ...
  5. **Failure semantics.** `consecutive_failures` counts misses; `last_check_ok = false` means a **confirmed** outage — `FAILURE_THRESHOLD = 2` misses in a row. A single network hiccup never reaches that state: the retry inside `probe` is part of *one* check, not a second one.
