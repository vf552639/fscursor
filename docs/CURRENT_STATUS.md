...
- 2026-04-30 (**task33**): FastPanel domain operations + bulk full setup workflow delivered (stage 1 + key stage 2 UX).
  ...
  - **Domain APIs:** added endpoints for create-site (`site_only` mode), create-db, db-credentials, ssl-request/cancel/refresh, nginx override get/set, and `bulk-full-setup`. (`mark-ns-set` и `check-ns` были в этом списке ошибочно — роутов под них нет; звавшие их кнопки удалены в спринте 3, фаза 5. `bulk-full-setup` удалён при развороте на zero-knowledge; сегодня существует `POST /api/domains/full-setup`, и он только проставляет связки пачке доменов — зону Cloudflare и NS выполняет десктоп.)
  - **Celery tasks:** added `create_db`, `request_ssl`, `revoke_ssl`, `nginx_override`, `check_ns` (registrar API with DNS fallback), `refresh_ssl`/`refresh_ssl_all`, and `bulk_full_setup`. (**Gone:** ни одной из этих задач в `app/tasks/` больше нет — выполнение уехало в десктоп. Пришедший им на смену роут `full-setup` задач не ставит вовсе.)
  ...
  - **Registrars:** `BaseRegistrarService` now includes `get_nameservers`; implemented for Namecheap (`domains.getInfo`) and Hostiq (`GET /domains/{domain}`). (Namecheap-реализация была нерабочей: звала `domains.getInfo`, а разбирала элемент из ответа `domains.dns.getList`. Исправлена ветвью `feat/domains-cloudflare-match`, тогда же выставлена наружу командой `registrar_get_nameservers`. **Gone:** чтения NS через API регистратора больше нет ни в трейте `RegistrarService`, ни командой — «как есть» спрашивают у реестра (RDAP, `domain_registry_nameservers`). Причина не в поломке: у Hostiq DI-API v3 такого поля нет вовсе (`docs/HOSTIQ_API.md` §5), а у ручных провайдеров нет и API, — то есть путь отвечал у одного провайдера из двух. Регистратору осталась запись.)
  - **Domains UI:** new `DomainDetailModal` tabs (Overview/DB/SSL/Nginx/NS), expanded bulk toolbar actions (Refresh SSL, Check NS, Mark NS Set), and `BulkSetupWizard` + multi-task progress modal for full setup runs. (**Gone:** вкладок в карточке больше нет — NS слита в Overview ветвью `feat/domains-cloudflare-match`; три кнопки удалены в спринте 3; `BulkSetupWizard` и progress modal в репозитории отсутствуют. Актуальная раскладка — `docs/ARCHITECTURE.md` § Domains UI.)
