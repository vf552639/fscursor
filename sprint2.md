...
- [x] **Step 1: Собрать карту использования каждого поля**
...
Выполнено 2026-08-03, плюс дополнительно `grep -rn` по `desktop/src-tauri/src` (в шаге в исходном
плане не было — десктоп читает поля по строковому ключу из кэш-строки, TS/py-grep это не ловит).

- [x] **Step 2: Классифицировать**
...
Результат по каждому полю (все шесть — **используются**, ни одно не мёртвое):

| Поле | Пишется | Читается |
|---|---|---|
| `fastpanel_status` | `ServerCreate` (`frontend/src/pages/Servers.tsx:99`, ветка «Connect Existing Fastpanel») → `backend/app/services/server_service.py:36`. Установка (`install_fastpanel`) НЕ пишет его обратно на сервер — только читает из локального кэша для идемпотентности. | `desktop/src-tauri/src/commands/provision.rs:813` (гейт идемпотентности `install_fastpanel`); `backend/app/services/server_service.py:36`; `frontend/src/pages/ServerDetail.tsx:26-27,221`, `Dashboard.tsx:32`, `Servers.tsx:214` |
| `fastpanel_url` | `Servers.tsx:98` (create) | `ServerDetail.tsx:79` (deep-link payload) |
| `fastpanel_user` | `Servers.tsx:96` (create) | `ServerDetail.tsx:80` (deep-link payload) |
| `fastpanel_version` | только модель/схема (нет пути записи) | `ServerDetail.tsx:82` |
| `fastpanel_port` | только модель/схема (default=8888) | `ServerDetail.tsx:83` |
| `fastpanel_password_blob_id` | `server_service.py:39,64` (create/update) | `ServerDetail.tsx:200-201` (`RevealSecret`) |

Особый случай — `fastpanel_status`: путь записи есть (создание сервера через «Connect Existing
Fastpanel»), но сам `install_fastpanel` после успешной установки не PATCH-ит `/servers/{id}` — это
подтверждает находку Спринта 1 («guard идемпотентности сейчас частично неактивен для пути
install-over-SSH»), но поле НЕ мёртвое: у него есть и чтения (включая десктопный гейт), и запись
(в другом потоке). Read-but-partially-written ≠ dead.

- [x] **Step 3: Действие по результату**

Все шесть полей используются → **не трогаем**. **FastPanel-поля: все используются, удалений нет.**

- [x] **Step 4: Коммит (только если были правки)**

Правок нет → коммита нет.
...
- [ ] В metadata audit нет ключей с `password`/`token` (assert в тестах зелёный).
- [ ] TOTP-компромисс задокументирован (`docs/security/TOTP_STORAGE.md`) и помечен в модели.
- [ ] На сервере нет открытых плейнтекст-полей секретов, кроме задокументированного `totp_secret`
      (ZK-sweep grep чист).
- [ ] `server.notes` удалён (модель, схемы, bulk-import, фронт, миграция применена).
- [ ] Легаси SSL-email-pool удалён целиком; `pytest`/`tsc`/`vitest` зелёные; таблица дропнута.
- [ ] CSP `connect-src` берётся из `WEB_CSP_CONNECT_SRC` (проверено рендером конфига вне localhost);
      nginx-переменные не сломаны.
- [ ] Вкладка Encryption описывает Argon2id + XChaCha20-Poly1305 (тест guard зелёный; нет упоминаний
      AES-256-GCM/ENCRYPTION_KEY/SHA-256).
- [ ] FastPanel-поля: зафиксирован вывод верификации (используются / что удалено).
...
- Реализован целиком: **нет** (план; исполнение — задача-за-задачей по под-навыку).
- Что осталось: выполнить остальные Tasks; продуктовая фаза (Stage 5) — отдельный задел.
- **Task 10 (FastPanel-поля): все используются, удалений нет.** Grep-верификация (backend/py,
  frontend/ts-tsx, plus `desktop/src-tauri/src` по строковым ключам чтения кэш-строки) подтвердила
  чтения/записи всех шести `fastpanel_*` полей (детали — в разделе Task 10 выше). Единственный нюанс:
  `fastpanel_status` пишется при создании сервера (`Servers.tsx` connect-flow), но не обновляется
  автоматически по завершении `install_fastpanel` — это тот же неактивный write-back, что отмечался в
  Спринте 1; поле остаётся read+partially-written, не мёртвым. Код не менялся, коммита нет.
