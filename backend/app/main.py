...
# маркером `password`.
#
# Расхождения с десктопным списком намеренные, их два.
#
...
SECRET_NAME_MARKERS = (
    ...
    # именно поэтому. `fastpanel_url` отвергается валидатором схемы ровно тогда,
    # когда в значении сидит userinfo (`https://admin:s3cr3t@ip:8888/`), то есть
    # пароль панели: у этой ошибки `input` — сам пароль. Список про то, чьё
    # значение нельзя вернуть клиенту, а не про то, чьё имя похоже на секрет.
    "fastpanel_url",
)
