...
# маркером `password`.
#
# Расхождения с десктопным списком намеренные, их три.
#
...
# 3. `token` и `secret` — на вырост. Плейнтекст-полей с такими именами схемы
#    не объявляют (`api_token`/`api_secret` отбиваются как лишние), но
#    `ConfirmEmailRequest.token` живёт без `min_length` только до первого дня,
#    когда он кому-то понадобится, и в этот день утечка открылась бы молча.
#
