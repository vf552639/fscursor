...
#
# Про `site_user` легко ошибиться в обе стороны, поэтому подробно.
#
# Это НЕ состояние машины, а производная от имени домена: `make_site_user`
# (`ssh/fastpanel.rs`) — чистая функция (слаг до первой точки, `-`→`_`, обрезка
# до 12, `_usr`), и `create_site` зовёт её же. Значение, записанное старой
# машиной, побайтово равно тому, что сгенерирует новая. Довода «иначе
# провижининг промахнётся мимо сайта» НЕ существует, и раньше здесь стоял
# именно он: `site_exists` — это `test -d` по ЖИВОЙ сессии, то есть по НОВОЙ
# машине, и промах спокойно падает в `create_site`.
#
# Гасим по другой, более узкой причине: карточка печатает владельца сайта
# сравнением записанного со снимком (`siteOwnerSource` в `lib/domainDrift.ts`,
# `DomainSiteCard`). Оставленная колонка утверждала бы владельца, которого на
# новой машине никто не проверял, — рядом с обнулённым `fp_facts` это ровно то
# же враньё, что и FTP-логин прежнего сервера.
#
# ЦЕНА, и она настоящая: NULL убирает саму пробу. Ветка `site_user_existing =
# None` уходит в `create_site` без вопросов, а `sites create` на уже
# существующий сайт отвечает ненулевым кодом (`docs/FASTPANEL_CLI.md` §1.3,
# сверено вживую), и вывод команды на сервер не уезжает — снаружи это отказ
# `STEP_CREATE_SITE` без причины. То есть переразвёртывание на машине, где сайт
# уже есть, из тихого переиспользования становится отказом. Достижимо «увезли и
# вернули» и — куда вероятнее — кнопкой «Привязать» (фаза 4), которой домен
# привязывают к машине, уже держащей его сайт. Лечение не здесь: десктоп мог бы
# при NULL выводить имя через `make_site_user` и пробовать `site_exists` им.
#
# Чего здесь НЕТ и почему:
#
# * `status` — оставлен ради восстановления в провижининге:
#   `domain_write_back_body` читает ПРОШЛЫЙ статус и по нему поднимает `failed`
#   → `site_created`, когда прогон не дошёл до SSL (`provision.rs`). Сброс
#   сломал бы эту логику, а колонка вдобавок `NOT NULL`. Довод «жизненный цикл
#   домена, а не машины» тут НЕ работает и раньше стоял зря: `site_created` и
#   `active` — утверждения ровно про сайт на машине. Поэтому цена своя, и она
#   больше, чем у `last_provision_error`: `active` переживает переезд и при всех
#   прочих пустых колонках утверждает работающее развёртывание на машине,
#   которая домена не видела. Это буквально «нарисовать незнание здоровьем» —
#   принцип №6, которым обосновано всё остальное в этом списке;
...
_FORGOTTEN_ON_MOVE: Final[dict[str, None]] = {
    ...
    "site_user": None,
    "site_path": None,
    # NULL здесь не бесплатен: провижининг читает эту колонку и при пустой
    # берёт `8.1` (`provision.rs`), так что сайт, работавший на 8.2, на новой
    # машине заведётся на 8.1. Задеты только досхемные строки — сегодня колонку
    # не пишет ни один маршрут, — но у давнего пользователя они как раз есть.
    "php_version": None,
    "php_handler": None,
    ...
    # временно не назвать причину, и пара «`failed` без причины» хотя бы не врёт
    # В ПЛЮС — в отличие от пережившего переезд `active` (см. `status` выше).
    # Следующий прогон перепишет обе колонки разом.
