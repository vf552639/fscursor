...
def clean_host(host: Optional[str]) -> str:
    """Адрес в том виде, в каком его можно спрашивать у сети.

    Нормализацией хранимого значения не занимается никто: `ServerBase.ip_address`
    — обычный `str` без валидатора и без `str_strip_whitespace`, так что
    `" 203.0.113.10 "` из CSV-импорта доезжает до колонки как есть. Резолвер на
    таком адресе отвечает «имя не разрешилось» — то есть мониторинг двумя
    прогонами позже разбудит владельца живой машины, а ложное «упал» здесь
    самый дорогой исход.

    Поэтому чистит монитор, и одной функцией на все три места, где адрес
    участвует: перед опросом, в тексте уведомления и при сверке «тот ли адрес
    мы опрашивали» (`tasks/server_monitor_task.py`). Разъехавшись, эти три
    места дали бы вердикт по одному адресу с адресом другого в письме.
    """
...
async def probe(
    ...
    *,
    ...
    """
    # Пробелы срезаются с самого значения, а не только с того, по чему
    # принимается решение (почему — в `clean_host`). Колонка при этом не
    # трогается: монитор ничего не переписывает, он только читает.
...
async def _notify(
    ...
    # Адрес чистится тем же `clean_host`, что и перед опросом: пробелы из
    # CSV-импорта доезжают до письма и Telegram, и `Port  203.0.113.10 :22 did
    # not answer` читается как опечатка отправителя.
    where = f"{clean_host(server.ip_address)}:{server.ssh_port}"
    if transition == "down":
