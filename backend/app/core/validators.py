...
def is_valid_fastpanel_url(value: str) -> bool:
    ...
    # `@` в authority — это userinfo, то есть креды внутри значения.
    #
    # ВЕТКА НЕ ПОКРЫТА ТЕСТОМ, и это осознанно: проверка формы «хост:порт» ниже
    # отвергает такое значение и сама (`@` не входит в набор символов хоста),
    # поэтому удаление этих двух строк ничего не роняет. Держим их второй
    # линией: правило «userinfo — отказ» слишком важное, чтобы держаться на
    # побочном эффекте чужого регекса, который завтра расширят под IPv6 или
    # ещё какой-нибудь символ.
    if "@" in authority:
        return False
