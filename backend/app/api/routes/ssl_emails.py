...
async def list_ssl_emails(
    user: User = Depends(get_current_user_or_401),
    db: AsyncSession = Depends(get_db),
) -> list[SslEmailResponse]:
    items = await ssl_email_service.list_emails(db, user.id)
...
async def create_ssl_email(
    ...
    try:
        item = await ssl_email_service.add_email(db, data.email, data.usage_cap, user.id)
...
async def patch_ssl_email(
    ...
    item = await ssl_email_service.update_email(
        db, email_id, user.id, is_active=data.is_active, usage_cap=data.usage_cap
    )
    if item is None:
...
async def delete_ssl_email(
    email_id: int,
    user: User = Depends(get_current_user_or_401),
    db: AsyncSession = Depends(get_db),
) -> None:
    deleted = await ssl_email_service.delete_email(db, email_id, user.id)
    if not deleted:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "SSL email not found")
