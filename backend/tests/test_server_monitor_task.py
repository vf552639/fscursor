...
def test_worker_start_rechecks_the_whole_fleet(published_tasks):
    ...
    Дыра, ради которой сигнал заведён: расписание бьёт по слотам 00/06/12/18
    UTC, и воркер, поднятый в 06:09, промахнулся бы мимо слота на девять минут —
    до полудня парк стоял бы непроверенным. Это возможный сценарий, а не разбор
    случившегося: наблюдавшийся `last_check_at = NULL` у всех серверов был
    вызван неполным реестром моделей в воркере (см. `app/models/__init__.py`),
    и к расписанию отношения не имел.
    ...
    прогон после деплоя тихо перестаёт случаться. Аргументы — те же, что celery
    шлёт по-настоящему (`apps/worker.py`: `sender`, `instance`, `conf`).
    """
