...
* **постановка в очередь не входит в контракт создания.** С мёртвым Redis
  ...
  Мёртвый брокер бывает двух видов, и они проверяются порознь: тот, что
  **отказывает** (исключение сразу), и тот, что **молчит** (ответа нет вовсе).
  Второй опаснее и заметен только по времени — до появления границы ожидания
  запрос висел на нём десятками секунд.
...
async def test_a_silent_broker_is_given_up_on_instead_of_waited_out(monkeypatch, app_log):
    """Брокер молчит — перестаём его ждать, а не ждём до победного.

    Это не дубль теста выше: там брокер **отказывает**, здесь — **молчит**, и
    отличаются они тем, чего стоят. Отказ прилетает исключением сразу; молчание
    же измеряется, и до появления границы стоило дорого — замер на этом самом
    вызове: 19.8 с на закрытом порту, 19.8 с на неразрешимом имени и больше 100 с
    (в одном прогоне — больше 25 минут) на недостижимом хосте. `retry=False`
    снимает повторы публикации, но таймаута соединению не задаёт.

    Предмет — время, а не факт вызова: снятая граница не меняет ничего, кроме
    задержки, и любая проверка «поставлено / не поставлено» проехала бы мимо.

    Проверяется `enqueue_reachability_check`, а не `POST /api/servers`, и это
    осознанно. Граница живёт здесь; на уровне запроса её не видно за латентностью
    БД — на `NullPool` через пулер Supabase каждое обращение открывает новый
    коннект, и сам по себе `POST /api/servers` стоит здесь ~2.5 с. Утверждение
    про запрос утонуло бы в этом разбросе и ловило бы чужой регресс вместо
    своего; что запрос переживает мёртвый брокер и отдаёт 201, проверено выше.

    Границу тест подменяет своей, крошечной: предмет — что механизм срабатывает,
    а не какое число стоит в константе. Заглушка спит заведомо дольше границы и
    настоящих сокетов не открывает, так что тест быстрый и герметичный.
    """
    bound = 0.1
    stub_sleeps = 2.0
    monkeypatch.setattr(server_service, "PUBLISH_TIMEOUT_SECONDS", bound)

    def _broker_never_answers(self, args=None, kwargs=None, **options):
        time.sleep(stub_sleeps)

    monkeypatch.setattr(Task, "apply_async", _broker_never_answers)

    started = time.monotonic()
    await server_service.enqueue_reachability_check(uuid.uuid4())
    elapsed = time.monotonic() - started

    assert elapsed < stub_sleeps / 2, (
        f"ждали молчащий брокер {elapsed:.2f} с при границе {bound} с — "
        "граница ожидания не сработала"
    )
    warnings = [
        rec
        for rec in app_log.records
    ...
    assert warnings, "истёкшее ожидание брокера прошло бесследно"
    assert "не ответил" in warnings[0].getMessage(), (
        "молчание брокера записано в лог неотличимо от его отказа"
    )
