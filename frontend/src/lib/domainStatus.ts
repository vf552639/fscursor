 ...
 *
 * Цвета бейджа сюда пока НЕ переехали — они остались выражением в ячейке
 * таблицы (`pages/Domains.tsx`). Это осознанный предел: единственный потребитель
 * у них один, и вытаскивать их сюда стоило бы переписывать разметку ячейки, что
 * относится к разбору страницы на компоненты (план №4).
 */
