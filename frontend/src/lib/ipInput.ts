...
export function ipError(value: string, opts: IpErrorOptions = {}): string | null {
  ...
  // На исход «принять или отвергнуть» эта ветка не влияет никогда: всё, что
  // длиннее, формат и так не пройдёт. Она определяет ФРАЗУ — «must be at most 45
  // characters» действеннее, чем «Invalid IP address format», когда в поле
  // уехал абзац.
  //
  // Правильно записанный адрес до неё не дотягивается, но запас тут нулевой:
  // предел 45 стоит впритык под самый длинный валидный адрес — IPv6 со
  // встроенным IPv4-хвостом, «1111:2222:3333:4444:5555:6666:255.255.255.255»,
  // ровно 45 символов. `String(45)` в схеме выбран под него, так что менять
  // лимит вниз нельзя, даже если кажется, что «адреса короче».
