...
describe("domainStatusIsRoutine", () => {
   ...
   *
   * Утверждение здесь ДВУСТОРОННЕЕ, и второго теста ему не нужно: фильтр идёт
   * по всей лестнице, поэтому «рутина — ровно эти два» и «остальные ступени
   * рутиной не считаются» — одна и та же проверка. Отдельный цикл по шести
   * оставшимся статусам стоял тут и был вечнозелёным: упасть отдельно от этой
   * строки он не мог, а впечатление второго сторожа создавал. Домен,
   * застрявший в ожидании NS или в развёртывании, — незакрытая работа, и в
   * списке на двести строк её видно только по бейджу; сторожит это равенство
   * ниже.
   */
  it("рутина — только `new` и `active`, остальные ступени лестницы — нет", () => {
