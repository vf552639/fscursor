 ...
 * выражение.
 *
 * «До вызова» — это до конца вызова, а он может ждать человека: у
 * `sshExecWithHostKeyRetry` аргументы (вместе с паролем) удерживаются
 * замыканием всё время, пока пользователь отвечает на вопрос про ключ хоста, и
 * это по построению — повторять вызов больше нечем. Ограничение сверху здесь
 * одно: таймаут ожидания решения.
 *
