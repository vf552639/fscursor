...
/**
 * ⚠️ Три константы ниже — копии значений из `backend/app/core/validators.py`
 * (`PG_INT_MAX`, `OS_PRETTY_MAX_LEN`, `KERNEL_MAX_LEN`), и общего источника у
 * них нет: граница языков. Меняешь одну сторону — иди и посмотри на другую;
 * на той стороне про эту копию сказано там же, рядом со значениями.
 *
 * Цена расхождения несимметрична и потому неочевидна: сузишь колонку на
 * бэкенде — фронт продолжит слать перелившееся значение, а 422 отвергает ВСЁ
 * тело целиком (`ServerMetricsIn`), так что один длинный `PRETTY_NAME` будет
 * стоить не поля, а целого снимка.
 *
...
export function parseServerMetrics(output: string): ServerMetricsIn | null {
  ...
  if (cpuBefore && cpuAfter) {
    ...
    // Потолок 100 — та же граница, что `le=100` у `cpu_usage_pct` в
    // `backend/app/schemas/server.py` (см. предупреждение о копиях у
    // `MAX_METRIC_INT`): двинешь там — двинь и здесь.
  ...
  for (const line of sampleLines(sections, "disk")) {
    ...
    diskTotalKb = total;
    diskUsedKb = used;
    // Первая подходящая строка. Спрошен один маунт (`df -kP /`), и запись в
    // выводе ровно одна, так что выбирать в норме не из чего: `break` здесь не
    // разрешает спор, а закрывает разбор, как только нужное найдено. Лишние
    // строки за ней — это уже не записи `df`, а шум в общем stdout (см.
    // дискриминатор по последнему токену выше), и продолжать по ним незачем.
    break;
  }
