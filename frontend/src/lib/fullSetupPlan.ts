 ...
 *
 * Речь только про ЗАПИСЬ, и `registrarSupportsNsApi` ниже — гейт именно её.
 * Прочитать, куда домен делегирован СЕЙЧАС, можно у любого домена независимо от
 * провайдера: это делает реестр (`api/rdap.ts`). Погасший тумблер значит «мы не
 * пропишем NS сами», а не «мы не узнаем, куда домен смотрит».
 */
 ...
 *
 * Экспортируется ради второго счётчика доменов — вопроса «привязать N доменов
 * к серверу?» на карточке сервера. Своя копия этих семи строк там разошлась бы
 * с этой ровно так же незаметно, как расходятся копии нормализации имени.
 */
