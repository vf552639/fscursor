...
describe("parseServerMetrics — вывод живых машин", () => {
  ...
  /**
   * Нужную строку в секции диска опознаём по точке монтирования, а не по месту
   * в списке. Порядком это не решается в принципе: шум приходит и до записи, и
   * после, а числа в нём бывают правдоподобные — и «диск 0 ГБ» на карточке
   * ровно то, против чего построен весь дизайн отказов.
   */
  it.each([
    // Диагностика `df` со стоящими в хвосте числами — и она ПЕРЕД записью, так
    // что «берём первую подходящую» тут не спасает.
    ["диагностика перед записью", "df: /mnt/nfs: Stale file handle 1 2 3 4 5\n{row}"],
    // Чужой маунт после нашего: так выглядела бы секция, позови кто-нибудь `df`
    // без аргумента.
    ["чужой маунт после записи", "{row}\ndevtmpfs           1957764       0   1957764       0% /dev"],
    // Две записи про `/` (bind-mount поверх корня) — берём первую, а не наугад.
    ["вторая запись про тот же корень", "{row}\noverlay              102400   51200     51200      50% /"],
  ...
  /**
   * Нулевой размер корня отдают некоторые псевдо-ФС. Ноль в колонке — это
   * утверждение «диска нет», которого мы не измеряли; прочерк честнее.
   */
  it("нулевой размер корня — прочерк, а не «0 ГБ»", () => {
    const zero = UBUNTU_OUTPUT.replace(
      "/dev/vda1         41152736 8123456  30934784      21% /",
      "none                     0       0         0       0% /",
    );
    const parsed = parseServerMetrics(zero);
    expect(parsed?.disk_total_gb).toBeNull();
    expect(parsed?.disk_used_gb).toBeNull();
  });
});
