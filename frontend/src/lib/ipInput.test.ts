...
describe("ipError — IPv6 только там, где он разрешён", () => {
  ...
  it("самый длинный валидный адрес влезает в колонку ровно впритык", () => {
    // Шесть групп плюс встроенный IPv4 — 45 символов, ровно ширина
    // `servers.ip_address`. То есть `String(45)` выбран под этот случай, а не «с
    // запасом»: проверка длины срабатывала бы на настоящем адресе, будь лимит
    // хоть на символ меньше. Литералом, а не через `IP_ADDRESS_MAX_LEN`: сверка
    // константы с самой собой зеленела бы при любом её значении.
    const longest = "1111:2222:3333:4444:5555:6666:255.255.255.255";
    expect(longest.length).toBe(45);
    expect(ipError(longest, { ipv6: true })).toBeNull();
  });
  ...
  it("сломанные формы не проходят даже с флагом", () => {
    ...
    // Парный к нему и ровно на границе правила «≤7 групп при сжатии»: восемь
    // групп уже полны сами по себе, и сжимать за ними нечего — «::» обязано
    // означать хотя бы один опущенный ноль. Подмена `total <= 7` на `<= 8` без
    // этих двух входов проходит незамеченной.
    expect(ipError("1:2:3:4:5:6:7:8::", v6)).toBe("Invalid IP address format");
    expect(ipError("::1:2:3:4:5:6:7:8", v6)).toBe("Invalid IP address format");
