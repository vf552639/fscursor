...
describe("formatExpiry", () => {
  ...
  it("ровные сутки назад называет своим числом, а не вчерашним", () => {
    // Граница, на которой подпись и ошибалась: у отрицательной разности `floor`
    // уводит ОТ нуля, поэтому «сутки назад» и «час назад» давали одно и то же
    // число дней, и любой ровный день терял единицу. `expiry_date` приходит
    // датой без времени — то есть ровные сутки здесь не редкий случай, а
    // единственный, который вообще бывает у домена.
