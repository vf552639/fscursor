 ...
 * («Press Sync»), кнопка в шапке вкладки гоняет его по всему
 ...
 */
export const CF_SYNC_VERB = "Sync";

/**
 * Что действие делает и чего НЕ делает — тултипом на обеих кнопках.
 *
 * Глагол обещает больше, чем происходит: «sync» читается как двусторонняя
 * сверка — с созданием недостающих зон и снятием протухших привязок, — а
 * делается поиск зоны по имени и запись привязки там, где её нет. Менять глагол
 * нельзя (он держит связку с подсказкой в строке), поэтому обещание сужается
 * здесь.
 */
export const CF_SYNC_TITLE =
  "Look up Cloudflare zones by domain name and save the binding; already bound domains are left alone";

/**
 * Подсказка в строке: почему имя аккаунта показано, но выглядит иначе, чем
 * сохранённая привязка, и что с этим делать.
 */
export const CF_HINT_TITLE = `Matches a Cloudflare zone — not saved in the database. Press ${CF_SYNC_VERB}`;
