...
/**
 * Имя архива по умолчанию — той же ФОРМЫ, что у сервера (`BackupPaths::new` в
 * `ssh/backup_run.rs`): `<домен>-<штамп>.tar`, штамп в UTC.
 *
 * Именно формы, а не то же самое имя: штамп здесь берётся в момент открытия
 * панели, а серверный — уже после синхронизации кэша, коннекта и поиска бинаря
 * панели, так что секунды почти всегда разойдутся. Совпадения и не требуется:
 * файл на диске называет человек в панели сохранения, а по какому пути он лёг —
 * отвечает команда, и печатается на экране её ответ. Одинаковая форма нужна
 * ровно затем, чтобы имя читалось как имя бэкапа этого домена, а не как
 * случайная строка.
 */
