 ...
 *
 * Порогов протухания здесь тоже нет и быть не должно: «через сколько показание
 ...
 * число, и не знает, что именно устаревает: потому `formatAgoStale` и ПРИНИМАЕТ
 * `stale` аргументом, а не считает его сам. Заметка живёт в шапке, а не хвостом
 * файла: замыкающей нотой она была, пока `formatAgoStale` была последней, — а
 * стоило дописать функцию, и нота оказалась над той, к которой не относится ни
 * единым словом.
 */
...
/**
 * Слово «домен» при счётчике: «1 domain», «2 domains».
 *
 * Правило русского склонения («1 домен», «2 домена», «11 доменов») отсюда ушло
 * вместе с переводом вкладки Domains на английский, а не осталось мёртвым:
 * ветка, которую никто не рисует, — это код, который следующий читатель принял
 * бы за живой.
 *
 * Функция при этом осталась, хотя выродилась в один тернарник, и это не
 * инерция: вызывающих у неё три (`describeServerBinding`, `fullSetupPlan`,
 * `AssignServerDialog`/`AssignCloudflareDialog` через них и напрямую), а «1
 * domains» в подписи — опечатка, живущая вечно, ровно как «1 доменов» до неё.
 * Написанное по месту, единственное число теряется в первом же новом месте.
 */
export function domainWord(n: number): string {
  return n === 1 ? "domain" : "domains";
}
