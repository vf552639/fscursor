...
function EditRegistrarModal({ registrar, onClose }: { registrar: any; onClose: () => void }) {
  ...
  const patch = (blobIds: { apiKey?: string; apiSecret?: string }) => ({
    name: name.trim(),
    // Признак ТОТ ЖЕ, что в форме создания (`needsApiUser`): два правила про
    // одну колонку в двух формах — это ровно то расхождение, которым и начался
    // весь этот разбор. Нет поля — уезжает `null`, и уезжает он в PATCH явно.
    //
    // Это не «не показываем — значит не меняем»: здесь поле МЕРТВО ПО КОНТРАКТУ
    // провайдера. DI-API v3 авторизуется одним заголовком
    // `X-Authorization-Token`, `api_user` не участвует ни в одном запросе Hostiq
    // и не будет участвовать; у ручного ярлыка API нет вовсе. Унаследованный
    // email — не данные пользователя, а мусор от API, которого не существовало,
    // и он продолжает светиться подписью «Hostiq · admin@…» на карточке, то
    // есть врать про то, чем аккаунт настроен. Момент чистки выбран не наугад:
    // старый ключ Hostiq нерабочий, и в правку за новым токеном обязан зайти
    // владелец КАЖДОГО существующего аккаунта — колонка чистится тогда, когда
    // человек и так осознанно перенастраивает учётку, а не за его спиной при
    // переименовании.
    //
    // ГРАНИЦА, чтобы это не расползлось: правило «чего не показали, того не
    // трогаем» живо и применяется ко всему остальному. Ключ и Client IP так
    // вычищать НЕЛЬЗЯ — они не мертвы, а просто не тронуты, и «пусто» значит у
    // них «оставь текущий» (см. `handleSave`: ненабранное поле не попадает в
    // `secrets`, и прежний `*_blob_id` остаётся у сущности). Обнуляем только то,
    // чего у провайдера не бывает в принципе.
    api_user: showApiUser ? apiUser.trim() || null : null,
    ...(blobIds.apiKey ? { api_key_blob_id: blobIds.apiKey } : {}),
    ...(blobIds.apiSecret ? { api_secret_blob_id: blobIds.apiSecret } : {}),
  });
