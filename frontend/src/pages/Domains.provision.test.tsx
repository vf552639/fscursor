...
function renderPage(onProvisionResult: (r: any) => void = vi.fn()) {
  ...
  const ui = (
    <QueryClientProvider client={queryClient}>
      <Domains onProvisionResult={onProvisionResult} onBulkProvisionResult={vi.fn()} onBulkProvisionError={vi.fn()} />
    </QueryClientProvider>
  );
...
function Workspace({ rows }: { rows: Array<{ id: number; name: string }> }) {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <Domains onProvisionResult={queue.push} onBulkProvisionResult={vi.fn()} onBulkProvisionError={vi.fn()} />
      {queue.current && (
