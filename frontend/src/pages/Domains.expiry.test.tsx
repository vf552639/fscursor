 ...
 *
 * Имена подобраны так, что алфавит расходится и со сроком, и с датой заведения,
 * и с состоянием сертификата — иначе тест не отличил бы одну сортировку от
 * другой. Статусы у фикстур разные по другой причине: по ним считают чипы, а
 * непривычные (`ns_ok`, `teleported`) рисуют бейдж под именем домена.
 */
const DOMAINS = [
 ...
 *
 * Читается КНОПКА имени, а не текст ячейки целиком: под именем в той же ячейке
 * стоят метки (сервер, регистратор, а у застрявшего домена — ступень настройки)
 * и текст провалившегося прогона, и «delta.comNS set» не совпало бы ни с одним
 * ожиданием. Кнопка — первая в строке: чекбокс левее неё кнопкой не является.
 */
function rowNames(): string[] {
  return bodyRows().map((r) => r.querySelector("button")?.textContent?.trim() ?? "");
}
...
describe("Domains — сортировка по клику на заголовок", () => {
  ...
  /*
   * Двух проверок сортировки по ступени настройки здесь больше нет: колонка
   * «Setup» ушла из таблицы, кликать не по чему. Правило, которое они
   * сторожили («незнание в конец при ОБОИХ направлениях»), не осиротело — его
   * проверяют соседние случаи по сроку и по SSL, а незнакомый статус теперь
   * сторожат тесты строки (`DomainTable.test.tsx`): бейдж под именем ему
   * полагается наравне с провалом.
   */
  ...
  it("колонка SSL сортируется по тому же состоянию, которое и показывает", () => {
    ...
    fireEvent.click(sortBtn("SSL"));
    const desc = rowNames();
    // Второй клик поднимает отказ чтения — то, что и ищут вторым кликом.
    // А незнание остаётся внизу при ОБОИХ направлениях.
