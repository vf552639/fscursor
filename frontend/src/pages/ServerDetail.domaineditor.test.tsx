...
describe("ServerDetail — правка домена, поля дат", () => {
  ...
  it("в StrictMode форма слышит поле и не взводит на его рассказе ничего лишнего", async () => {
    setTauri(true);
    mocks.apiPut.mockResolvedValue({ ...DOMAIN });
    ...
    // мёртвой кнопкой на пустом месте — у домена без дат сохранять как раз
    // нечего.
    const f = await openEditor(
    ...
    // Набранное поверх — вторая половина теста, и она несущая: у домена без дат
    // рассказ поля СОВПАДАЕТ с начальным состоянием формы, поэтому одними
    // пустыми полями «услышала» форма ответ или пропустила мимо — не различить.
    // Дата, которой в домене не было, приходит ровно из `onParsed`, и другого
    // пути в payload у неё нет.
    type(f.purchase, "01.02.2027");
    fireEvent.click(saveBtn());
    await waitFor(() => expect(mocks.apiPut).toHaveBeenCalledTimes(1));
    const body = mocks.apiPut.mock.calls[0][1];
    expect(body.purchase_date).toBe("2027-02-01");
    // А нетронутое поле осталось пустым — двойное монтирование ничего в него
    // не занесло.
    expect(body.expiry_date).toBeNull();
  });
  ...
  it("нечитаемая дата из базы выключает Save, называет причину и выпускает по перенабору", async () => {
    ...
    // Вслух и вежливой очередью: выключенная кнопка выпала из tab-порядка, и в
    // режиме обзора до строки ещё надо добраться, а assertive окликал бы
    // человека посреди правки соседнего поля.
    expect(screen.getByText(BLOCKED).getAttribute("aria-live")).toBe("polite");
