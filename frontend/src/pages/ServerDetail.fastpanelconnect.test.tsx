...
// Не своя копия правила: утверждение «форма забракует собранный из IPv6 URL»
// должно опираться на ту самую проверку, а не на её пересказ в тесте.
import { fastpanelUrlError } from "../lib/fastpanelInput";
...
describe("ServerDetail — подключение существующей панели", () => {
  ...
  it("у IPv6-сервера поле URL открывается пустым, а не заведомо невалидным", async () => {
    setTauri(true);
    ...
    // Собранный из IPv6 адрес эта же форма и забракует: `fastpanelUrlError`
    // знает только «хост:порт» без скобок — и так же устроены обе парные
    // проверки (бэкенд, десктоп), так что подключить панель по IPv6 сегодня
    // нельзя вовсе. Предзаполнить поле значением, которое форма отвергнет, —
    // худший способ об этом сообщить; пустое поле с плейсхолдером честнее.
    expect(form.url.value).toBe("");
    expect(fastpanelUrlError("https://[2a01:4f8:c17:b8f::1]:8888")).toBeTruthy();
  });

  it("в карточке доступа IPv6 всё же берётся в скобки", async () => {
    setTauri(true);
    // Тут адрес не проверяется, а ОТКРЫВАЕТСЯ в браузере, и без скобок
    // «https://2a01:4f8:c17:b8f::1:8888» разбирается как хост «2a01» с портом
    // «4f8» (RFC 3986). Свой `fastpanel_url` у сервера не записан — карточка
    // собирает адрес из IP.
    renderDetail({ ...SERVER_FP, fastpanel_url: null, ip_address: "2a01:4f8:c17:b8f::1" });

    expect(await screen.findByText("https://[2a01:4f8:c17:b8f::1]:8888")).toBeTruthy();
  });
