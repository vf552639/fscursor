 ...
 *
 * Считаем через `DomainStatusBadge`: он не мемоизирован сам, то есть его вызовы
 * и есть рендеры строк. Но рисуется он не у всех — только у домена, чья ступень
 * настройки не рутинная, — поэтому у фикстур статус `failed` и поэтому первый
 * тест сверяет счётчик с числом строк. Без этой сверки достаточно поставить
 * фикстурам `active`, чтобы счётчик замер на нуле, а все проверки вида «дельта
 * равна нулю» позеленели навсегда.
 */
...
const DOMAINS = Array.from({ length: ROWS }, (_, i) => ({
  ...
  // Не `active`: рутинному статусу бейдж не полагается, а счётчик рендеров
  // строк считает именно его вызовы (см. преамбулу файла).
  status: "failed",
  registrar_id: null,
  server_id: null,
...
describe("Domains — цена одного действия в рендерах строк", () => {
  it("ввод в поиске, ничего не отфильтровавший, не перерисовывает ни одной строки", async () => {
    ...
    // Счётчик правда считает строки, а не молчит: перестань бейдж рисоваться —
    // и все дельты ниже стали бы нулевыми при любой поломке мемоизации.
    expect(rowRenders()).toBe(ROWS);
