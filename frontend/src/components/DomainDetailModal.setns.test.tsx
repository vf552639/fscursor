...
describe("вкладки не разводят Cloudflare и nameservers", () => {
  it("не предлагает DB / SSL / NGINX и Create Site и не ходит по их роутам", async () => {
    ...
    // Список целиком — намеренно, хотя ровно те же пять вкладок сторожит
    // `DomainDetailModal.tabs.test.tsx`. Утверждения разные: там — «строка
    // вкладок вот такая», здесь — «вкладки под вопрос про NS нет ПОД ЛЮБЫМ
    // именем». Строка выше ловит только буквальное «NS», а вернуться этот
    // вопрос может «Nameservers», «DNS» или «Delegation» — их поймает лишь
    // равенство всему списку. Цена — одна строка правки на каждую новую
    // вкладку, и правка эта полезная: она заставляет вписать новое имя сюда
    // руками, то есть посмотреть, не отвечает ли оно на вопрос, который уже
    // решён на Overview.
