 ...
 *    ОДИН раз и раздаёт вниз (сегодня — шапке; секция фактов переедет на то же
 *    значение в фазе 3), а непроверенный домен получает серое «Not checked», а
 *    не зелёное.
...
describe("шапка карточки", () => {
  ...
  it("непроверенный домен получает серое «Not checked», а не зелёное", () => {
    ...
    // Бейдж шапки карточка считает один раз (`sslState`) и отдаёт пропсом;
    // секция пока считает своё той же функцией с теми же аргументами — второй
    // потребитель переедет на общее значение в фазе 3, когда карточка SSL
    // выйдет из секции наружу. До тех пор этот тест и есть то, что держит два
    // ответа вместе.
