 ...
 *    provision печатается приглушённым И с подписью под каждым полем: гасит её
 *    только легенда секции сервера (`RecordedNoteInLegend`), которой здесь нет.
 *
...
describe("свежесть снимка — в шапке карточки", () => {
  ...
  it("проваленная проверка не молодит снимок: возраст считается от fp_facts_at", () => {
    ...
    // карточки (`facts/snapshot`), и держится оно ровно этим утверждением.
...
describe("снимка не было ни разу", () => {
  it("известное из provision показано как наша запись — и сказано СЛОВАМИ", () => {
    ...
    // Подпись печатается под КАЖДЫМ полем, и это главное утверждение теста.
    // Легенда «Приглушённые значения — из provision, на сервере не проверено»
    // принадлежит секции сервера и на Overview не рисуется вовсе — значит гасить
    // приписку тут нечем и незачем: без неё смысл приглушённого значения держался
    // бы на одном сером цвете, который миграция на токены ещё переназначит.
