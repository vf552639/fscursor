 ...
 * карты, а не выписан рядом: новая ступень приедет в таблицу сама, а не будет
 * забыта — и число ступеней тут нигде не названо, чтобы не разойтись с ними.
 */
...
describe("состояние сертификата названо словом", () => {
  it("фикстура и правда даёт COMPUTED, иначе пропуск строки съедет молча", () => {
    // `COMPUTED` — утверждение о ФИКСТУРЕ, и до этой строки его не проверяло
    // ничто. Поменяй `facts()` — и пропуск сработал бы не на той ступени, а
    // мутанты продолжили бы умирать на остальных: тест деградировал бы, не
    // упав, то есть ровно тем способом, который эта фаза и ловит.
    expect(sslState(fresh.fp_facts.ssl, fresh.fp_facts_at, Date.now())).toBe(COMPUTED);
  });
