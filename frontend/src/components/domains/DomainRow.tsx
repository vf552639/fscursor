...
function DomainRow({
  ...
  >
    ...
    {/* Тег сертификата ровно о двух состояниях: он есть и он живой — либо нет.
        Точный `ssl_status` (`pending` / `error` / пусто) уехал в `title`, и это
        не потеря: разница между «сертификат не выпустился» и «его не
        заказывали» настоящая, но она про ОДИН домен, а колонка отвечает на
        вопрос, который задают ко всему списку — «где сайт открывается по
        https». Место для разбора есть на карточке (`DomainSslCard`, вкладка
        Overview), и там оно на своём месте. Молчать о разнице всё равно нельзя,
        поэтому она в подсказке, а не выброшена.

        Подстрочника со сроком сертификата тут больше нет: сроков в строке было
        два, и рядом они спорили — колонка «Expires» про домен, а подпись под
        тегом про сертификат, и читались они как один. */}
