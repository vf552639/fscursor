  ...
  return (
    <TabBody>
      ...
      <div style={{ fontSize: 13, color: "#374151", display: "grid", gap: 6 }}>
        {/* Строки «NS: pending (auto)» здесь больше НЕТ.
            Это была наша запись о последней попытке смены NS плюс
            `ns_check_mode`, и стояла она двумя рядами выше карточки
            NAMESERVERS — той самой, что сверяет делегирование ЖИВЬЁМ (CF +
            RDAP). Два разных ответа про NS на одном экране, и верхний из них
            слабейший: он говорит, что мы делали, а не что там на самом деле.
            Довод «одна говорит про нас, вторая про мир» верен, но цена его —
            человек, читающий два ответа и выбирающий между ними наугад.

            Следствие названо честно: `ns_check_mode` теперь не показывается
            нигде (в UI он и не писался никогда), а наша запись `ns_status`
            видна только счётчиками `NS details` на вкладке Domains. */}
