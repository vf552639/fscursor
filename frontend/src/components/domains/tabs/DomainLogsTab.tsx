 ...
 * приложении, но причина уже другая, чем была. Прежняя («вебфонт ради одной
 * модалки — лишняя сетевая зависимость в Tauri») больше не действует:
 * `IBM Plex Mono` подключён пакетом `@fontsource`, лежит в бандле и сети не
 * требует. Осталась вторая половина возражения: чем набраны моноширинные места,
 * — вопрос уровня всего приложения, а не этой модалки, и решать его надо разом
 * для логов, путей, айди и токенов. Пока не решён, системный `ui-monospace`
 * остаётся здесь намеренно, и у подключённого `IBM Plex Mono` сегодня нет ни
 * одного потребителя — он ждёт этого решения, а не забыт.
 *
      ...
      {logs.length > 0 && selected ? (
        <>
          ...
          {/* Полный путь выбранного файла. Моно — потому что путь читают
              ...
              приложении), а не `IBM Plex Mono` из макета, и это решение, а не
              недоделка: сам шрифт теперь в бандле (`@fontsource`, импорты в
              `main.tsx`), сетевой зависимостью он быть перестал, но переводить
              на него моноширинные места надо разом по всему приложению —
              логи, пути, айди, токены, — а не в одной модалке. См. тот же
              разбор у консоли выше. */}
