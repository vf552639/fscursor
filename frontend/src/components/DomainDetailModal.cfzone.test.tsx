...
describe("делегирование — три состояния и «не знаем»", () => {
  it("делегировано — по ответу РЕЕСТРА, и вопрос задан только про имя домена", async () => {
    ...
    expect(new Set(mocks.invokeSynced.mock.calls.map((c: any[]) => c[0]))).toEqual(
      // Второе чтение — записи зоны: поле сервера сверяет A-запись apex с
      // адресом выбранного сервера. Оба имени здесь ЧИТАЮЩИЕ и оба про
      // Cloudflare; утверждение по-прежнему в том, что ключами регистратора
      // карточка не ходит вовсе.
      new Set(["cf_list_zones", "cf_list_dns_records"]),
    );
