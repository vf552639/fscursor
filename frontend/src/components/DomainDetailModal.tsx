    ...
    <Modal title={`Domain: ${domain.domain_name}`} onClose={onClose} width={760}>
      ...
      {tab === "overview" && (
        /* Два столбца: слева — чем домен является (учётки, статус, сроки),
           ...
           то есть менять контракт компонента. Планом про разбор страницы это
           больше не числится: он выполнен, а карточку не трогал — она осталась
           в `components/`, и рендерит её всё та же `pages/Domains.tsx`. Долг
           живой и ничей: id вместо имени читателю ничего не говорит. Цена его
           при этом упала — три списка страница уже держит (она раздаёт их
           `DomainTable`), так что сегодня это три пропа, а не три запроса.
