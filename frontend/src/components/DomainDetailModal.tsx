 ...
 * содержимое пока не читает никто, и вкладка говорит это словами. **Template**
 * и **Custom** — заглушки: продукт не знает ни шаблонов сайта, ни сохранённых
 * многошаговых действий, и обе вкладки называют это словом.
 *
 ...
 *
 * Вкладок здесь ровно пять — как в макете, — и ни одна не открывается в
 * пустоту. Правило про содержимое от этого не отменилось, оно уточнилось:
 * запрещена не «вкладка без функции», а вкладка, которая функцию ОБЕЩАЕТ, —
 * пустая панель или, того хуже, живые с виду органы управления, ничего не
 * делающие. Overview, Server и Logs приехали с данными; Template и Custom —
 * с одной карточкой-заглушкой каждая (`COMING SOON` и фраза о том, чем вкладка
 * станет), той же формы, что заглушки DMCA и Backups внутри соседних вкладок.
 * Незнание — состояние, названное словом, а не пустое место (принцип №6
 * CLAUDE.md).
 *
  ...
  return (
    ...
    >
      {/* Порядок вкладок — из макета и означает путь сужения вопроса: что
          домен есть → что развёрнуто → что он пишет в логи → чем он застроен →
