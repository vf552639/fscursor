 ...
 *
 * 1. Ни одна вкладка не открывается в пустоту. Overview, Server, Logs и Backup
 *    приехали с данными — их содержимое проверяется у самих вкладок
 ...
 *    панель.
 *
 *    Backup — из первой группы, а не из второй, хотя органов управления на ней
 *    сегодня нет ни одного: она читает снимок сервера и говорит словами то,
 *    чего в нём нет, а не обещает будущую функцию пилюлей.
...
describe("строка вкладок", () => {
  it("вкладок ровно шесть, порядок — данные слева, заглушки справа, открыта Overview", () => {
    // Порядок проверяется целиком, а не набором: Backup стоит между данными и
    // заглушками, и переехав за Template, вкладка с настоящим содержимым
    // оказалась бы там, куда человек перестаёт заглядывать.
    show();
  ...
  it("Backup открывается своим содержимым, а не пустой панелью", () => {
    // Как и у Logs: вкладка в строке и панель под ней — разные вещи. У домена
    // фикстуры снимка нет, поэтому вкладка обязана сказать это словами, а не
    // показать пустоту. Правила самой вкладки — в её файле тестов.
    show();
    openTab("Backup");
   ...
   * комбобоксов и полей — нет. Тем же приёмом проверялась и заглушка Backups,
   * пока резервные копии не получили свою вкладку
   * (`domains/tabs/DomainBackupTab.test.tsx`).
   *
