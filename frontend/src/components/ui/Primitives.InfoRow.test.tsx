 ...
 * Information. Проверяем только сам примитив, а не его будущего потребителя
 * (Name/IP/Provider/OS на `ServerDetail` станут редактируемыми отдельной
 * задачей): без `onEdit` карандаша нет и разметка не меняется, с `onEdit` —
 * ровно одна кнопка с осмысленным доступным именем, и клик по ней зовёт
 * переданный колбэк, не трогая значение строки.
 */
