...
describe("DateField", () => {
  ...
  it("значения нет — поле пустое, и наружу это сказано как «значения нет»", () => {
    ...
    // Красного нет — и это про МОНТИРОВАНИЕ, а не про отложенный показ:
    // сторожится тем, что эффект монтирования только рассказывает наружу и
    // `setError` не трогает вовсе. Правило «молчать посреди набора» сторожат
    // тесты про набор, ниже.
  ...
  it("нечитаемое семя — пустое поле, но НЕ молчание", () => {
    ...
    // Красного при этом НЕТ, и это не «ошибка ждёт своего момента», как тут
    // было написано: момент не наступит вовсе — строку из базы никто не
    // набирает, а `setError` живёт только в обработчике изменения. То есть
    // молчание тут вечное, и сторожит эта строка ровно его: красное на
    // монтировании не рисуется никогда, каким бы ни было семя.
    //
    // Отсюда обязанность ПОТРЕБИТЕЛЯ: раз поле молчит, объясниться должен он —
    // на то ему и отдельный `onParseErrorSettled` (тест ниже). Так и сделано у
    // обоих: строка про мёртвый Save на карточке сервера и подпись под полем в
    // шапке карточки домена.
    expect(anyError()).toBeNull();
  });
