...
describe("formatBytes", () => {
  ...
  it("на границе ступени печатается следующая единица, а не «1024» предыдущей", () => {
    // Округление стоит после выбора единицы, поэтому у каждой границы есть
    // щель: без её починки байт без одного до мегабайта печатался как «1024 KB»
    // — единица, которой в ряду нет, и глазами она читается как «в тысячу раз
    // больше соседней строки».
    expect(formatBytes(1048575)).toBe("1 MB");
    expect(formatBytes(1024 ** 3 - 1)).toBe("1 GB");
    expect(formatBytes(1024 ** 4 - 1)).toBe("1 TB");
  });
