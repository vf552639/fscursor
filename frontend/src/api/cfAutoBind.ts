 ...
 *
 * Живёт вне React намеренно: вызывающих трое и ни один не хук — автопривязка
 * сразу после создания домена (модалка к этому моменту уже закрывается) и две
 * кнопки «Синхронизировать»: по всему списку из шапки вкладки и по выделенным
 * из тулбара. Хук здесь означал бы, что прогон умирает вместе с компонентом,
 * который его начал.
 *
...
export interface CfBindNotice {
  /**
   * `warn` — не всё, что прогон брался привязать, привязано (те же два тона,
   * что у тостов воркспейса). `info` — привязывать было нечего либо привязано
   * всё; только в этом случае баннер имеет право сказать «✓».
   */
  kind: "info" | "warn";
  text: string;
}
 ...
 *
 * Про выделение фраза молчит намеренно: набор приносит вызывающий, и у одного
 * из трёх он — весь список (кнопка синхрона в шапке вкладки выделения не
 * спрашивает). «Все 12 выделенных доменов уже привязаны» при пустом выделении
 * описывало бы экран, которого нет.
 */
function nothingToDo(skipped: number): CfBindNotice {
  return {
    ...
    text:
      skipped > 0
        ? `Cloudflare: nothing to do — all ${skipped} domain(s) are already linked.`
 ...
 *
 * `mode` разводит вызывающих, и разводит по существу:
 ...
 * - `manual` (кнопка синхрона, любая из двух) обязан ответить всегда: кнопка,
 *   которая молчит, не отличима от сломанной.
 */
export function summarizeCfBind(
  ...
  // Тон решает ОДИН вопрос: осталось ли непривязанным что-то из того, что
  // прогон брался привязать. «✓» имеет право появиться только там, где ответ
  // «нет» — привязано всё, что рассматривали (либо рассматривать было нечего,
  // и это ветки `nothingToDo` выше).
  //
  // `none` считается наравне с остальными тремя причинами, и это исправление:
  // без него «0 of 5 linked, 5 with no matching zone» приезжало `info` и
  // рисовалось зелёной галочкой — прогон не привязал ни одного домена, а глиф
  // сообщал, что всё в порядке. По кнопке (`manual`) это единственный видимый
  // исход такого прогона. Особого статуса у «зоны не нашлось» нет: для
  // пользователя это ровно такой же непривязанный домен, как неоднозначный или
  // незаписанный, и делать ему нужно то же самое — идти смотреть руками.
  // Отдельная причина не сливать `none` с остальными — только в ТЕКСТЕ (своё
  // число), а не в тоне.
  //
  // Шум это не создаёт: после создания домена (`auto`) прогон, у которого нет
  // ничего, кроме `none`, вообще молчит — см. `changedOrHid` выше.
  const allLinked =
    report.none.length === 0 &&
    report.ambiguous.length === 0 &&
    report.writeFailed.length === 0 &&
    report.unreadAccounts.length === 0;
  return { kind: allLinked ? "info" : "warn", text };
}
