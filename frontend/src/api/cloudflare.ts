...
export interface CloudflareAccountCreate {
   ...
   * Поля `api_token` здесь нет намеренно: серверная схема его не объявляет, и
   * до `extra="forbid"` она молча его выбрасывала (200 OK и `NULL` в колонке) —
   * в TS оно давало зелёную компиляцию на пути, который теряет секрет. Теперь
   * это ошибка типа здесь и 422 `extra_forbidden` там.
   */
  api_token_blob_id?: string | null;
  is_active?: boolean;
}
...
export function useCloudflareZones(accountId: number | null | undefined) {
  return useQuery({
    ...
    // но причина у каждого соседа своя, и «выравнивать» их под один флаг
    // нельзя именно поэтому.
    //
    // `useDnsRecords`: текст его ошибки И ЕСТЬ объяснение, которое веб
    ...
    // Второй его потребитель — поле сервера на карточке домена
    // (`DomainServerField`) — рассуждает наоборот и гасит запрос У СЕБЯ, передав
    // `null` вместо аккаунта: сверка с A-записью там довесок, показывать её
    // ошибку негде, и в вебе она не предусмотрена вовсе. Отсюда и правило: флаг
    // ставит тот, кто знает, что делать с ошибкой, а не сам хук.
    //
