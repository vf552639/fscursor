...
export interface MultiSecretSave<K extends string> extends SecretFormState {
   ...
   * Это про ПРАВКУ: на создании ключ обязателен всегда, пропущенный там даёт
   * сущность с `*_blob_id = NULL` — тот самый 200 OK и неработающие команды.
   *
 ...
 * срабатывал бы на честных параллельных формах. Остаток риска есть: две формы
 * с пересекающимися `persist` столкнутся — окно равно одному запросу
 * сохранения сущности, а не всей операции.
 */
